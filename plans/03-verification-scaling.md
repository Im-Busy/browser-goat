...
Phase 3 adds the Verification Layer (Layer 6), making Browsing-Meta reliable for high-stakes queries where correctness matters more than speed. It runs multiple search rollouts with variation, votes on answers, and uses an LLM verifier for consensus.
...
class MultiRollout:
    ...
    async def execute(
        self, query: str, meta: BrowsingMeta, num_rollouts: int = DEFAULT_ROLLOUTS
    ) -> list[SearchResult]:
...
Add a `reliability_mode` to `BrowsingMeta.search()`:
