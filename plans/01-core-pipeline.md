...
| 1.0.3 | `src/browser_goat/models.py` | 2 hours | Pydantic schemas: `SearchResult`, `ExtractedSource`, `QueryIntent`, `ReliabilityInfo`, `SearchRequest` |
| 1.0.4 | `src/browser_goat/searxng_client.py` | 2 hours | Async httpx client for SearXNG JSON API: `search(query, engines, time_range, language) -> list[RawSearchResult]` |
...
**Goal**: Wire all layers together into a single `BrowserGoat.search()` call.

#### Subtask 1.5.1 — Main Router (`src/browser_goat/router.py`)
**Effort**: 6 hours
...
- [ ] `BrowserGoat.search("query")` returns a complete `SearchResult`
